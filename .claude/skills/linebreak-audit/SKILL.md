...
> 이 스킬 디렉터리(아래 `SKILL_DIR`)는 `.claude/skills/linebreak-audit/`. 경로는 이 기준.
...
가능하면 `Agent` 도구로 **전용 평가 에이전트**를 띄운다(스폰 불가 환경이면 인라인으로 동일 절차):

> `SKILL_DIR/agents/linebreak-auditor.md`와 `SKILL_DIR/references/linebreak-rubric.md`를 Read로
