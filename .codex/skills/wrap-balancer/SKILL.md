...
description: >-
  ...
  무의존성 바닐라 포트)를 CDN script tag로 주입해 줄바꿈을 균형 잡고, 한국어로 적용 전/후를
  렌더링·스크린샷해 전/후 시각 변화와 전용 루브릭 평가 에이전트로 개선
...
2. **검증** — 한국어 텍스트로 **적용 전/후를 렌더링·스크린샷**해서, 이미지/수치 기반
   시각 변화 확인(무엇이 바뀌었나)과 **전용 루브릭 평가 에이전트**(개선됐나·몇 점인가)로 평가한다.

밸런싱은 "줄 수를 늘리지 않으면서 각 줄 너비를 고르게, 마지막 줄 외톨이를 없애는" 것이 목표다.
줄바꿈이 눈에 띄게 바뀌는 것은 실패가 아니라 **성공 신호**다.

> 이 스킬 디렉터리(아래 `SKILL_DIR`)는
> `.codex/skills/wrap-balancer/`. 경로는 이 기준으로 읽는다.
...
**(A) 시각 변화 확인 — "무엇이 어떻게 바뀌었나"**

`before.png`, `after.png`, 가능하면 `metrics.json`을 함께 열어 줄 수·줄별 너비·마지막 줄 상태를
비교한다. Browser 스크린샷, `view_image`, 또는 사용 가능한 이미지 비교 도구를 써도 되지만,
시각 비교의 목적은 합격 판정이 아니라 **전→후 시각 변화 목록**을 만드는 것이다.

```
inputs:
  before: <WORKDIR>/harness/before.png
  after: <WORKDIR>/harness/after.png
  metrics: <WORKDIR>/harness/metrics.json
...
**루브릭 기반 채점**을 한다. 가능하면 Codex native subagent로 별도 평가자를 띄워 다음을 시킨다
(서브에이전트 스폰이 불가한 환경 — 중첩 호출 제한·Codex App 등 — 이면 **인라인으로** 동일
절차를 직접 수행한다):

> `SKILL_DIR/agents/balance-quality-evaluator.md`와 `SKILL_DIR/references/balance-rubric.md`를
> 파일로 읽고, `<WORKDIR>/harness/before.png`·`after.png`(있으면 `metrics.json`)를 근거로
...
- 사용자에게: 적용한 변경 요약 + 전/후 스크린샷(또는 compare.html 경로) + 루브릭 verdict·점수 +
  시각 변화 differences + 필요한 suggestions.
...
## 시각 변화 확인 vs 루브릭 에이전트 (왜 둘 다인가)

| | 시각 변화 확인 | balance-quality-evaluator (이 스킬) |
|---|---|---|
...
둘은 보완 관계다. 시각 변화 확인으로 "무엇이 바뀌었는지" 서술하고, 루브릭 에이전트로 "그 변화가
개선인지" 판정한다.
